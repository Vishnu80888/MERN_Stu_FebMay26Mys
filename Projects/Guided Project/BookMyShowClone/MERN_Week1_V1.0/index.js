...
bookingEmitter.on("bookingCancelled", (booking) => {
    console.log(
        chalk.yellow(
            `Booking Cancelled for ${booking.movieTitle} at ${booking.time}`
        )
    );
});

bookingEmitter.on("bookingFailed", (error) => {
    console.log(chalk.red(`Booking failed: ${error.message || error}`));
});
...
function handleInvalidAttempt(message) {
    ...
    console.log(
        chalk.yellow(`Attempts left: ${MAX_ATTEMPTS - invalidAttempts}`)
    );
...
function showMenu() {
    ...
    console.log("2. Book Tickets");
    console.log("3. View Booking");
    console.log("4. Cancel Booking");
    console.log("5. Re-Book Tickets");
    console.log("6. Exit");
...
function startBookingFlow() {
    ...
    rl.question("\nEnter movieId: ", (movieIdInput) => {
        ...
        validateMovieSelection(movies, movieId, (movieError, selectedMovie) => {
            ...
            rl.question("Enter time slot: ", (timeInput) => {
                validateTimeSelection(
                    selectedMovie,
                    timeInput,
                            ...
                            validateSeatCount(
                                ...
                                async (seatError, validSeatCount) => {
                                    ...
                                    try {
                                        ...
                                        if (!handleInvalidAttempt(error.message || error)) {
                                            showMenu();
                                        }
                                    }
                                }
